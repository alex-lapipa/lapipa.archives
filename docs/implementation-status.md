...
- First accession evidence: `LP-ACC-2026-0001` and valid BagIt submission package `LP-BAG-2026-0001` contain the 2019 origin-deck PDF (1 file; 194,031,448 bytes). Original and package SHA-256 values match. A complete ClamAV 1.5.3 scan using current official databases found no threats, and qpdf 12.3.2 validation passed with five documented recoverable warnings. Managed-storage ingest, file-object registration, rights review, independent replication, and restore testing remain pending.
...
- Migration provenance: canonical migration filenames match all 22 versions in the live Supabase migration ledger; no local-only or remote-only canonical version remains.
