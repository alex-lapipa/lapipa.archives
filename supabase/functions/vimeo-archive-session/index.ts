...
async function createCode(req: Request, body: Record<string, unknown>): Promise<Response> {
  ...
  if (body.video_id !== ACCEPTANCE_VIDEO_ID) return response({ error: "outside_acceptance_scope" }, 400);
...
async function vimeoDownload(body: Record<string, unknown>): Promise<Response> {
  ...
  if (session.video_id !== ACCEPTANCE_VIDEO_ID) return response({ error: "outside_acceptance_scope" }, 403);

  const fields = [
    "uri", "name", "description", "duration", "created_time", "modified_time",
    "release_time", "privacy.view", "download",
  ].join(",");
  const provider = await fetch(`https://api.vimeo.com/videos/${ACCEPTANCE_VIDEO_ID}?fields=${encodeURIComponent(fields)}`, {
    headers: {
  ...
  if (!provider.ok) {
    console.error("Vimeo acceptance lookup failed", { status: provider.status, video_id: ACCEPTANCE_VIDEO_ID });
  ...
  return response({
    ...
    video: {
      ...
      title: typeof video.name === "string" ? video.name : "Subterranea @ LA PIPA :: VIUDA",
    ...
    download: {
      ...download,
      filename: `vimeo-${ACCEPTANCE_VIDEO_ID}-source.${download.file_extension}`,
    },
  });
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (error instanceof RangeError) return response({ error: "request_too_large" }, 413);
    const message = error instanceof Error ? error.message : "unknown";
    if (message.includes("42501") || message.includes("403")) return response({ error: "forbidden" }, 403);
    if (message.includes("22023") || message.includes("400")) return response({ error: "invalid_or_expired_capability" }, 400);
    console.error("Vimeo archive session failed", { error: message.replace(/[A-Za-z0-9_-]{20,}/g, "redacted") });
